...
app = Flask(__name__)
CORS(app)  # Menambahkan CORS untuk mengizinkan akses lintas domain

# Membuat direktori untuk menyimpan hasil crop wajah
os.makedirs("cropped_faces", exist_ok=True)

# Fungsi untuk menghitung jarak antara dua embedding
...
def authenticate():
    try:
        ...
        if image1 is None or image2 is None:
            return jsonify({"error": "Gambar tidak valid atau tidak dapat diproses"}), 400
        ...
        embedding2 = DeepFace.represent(image2, model_name="VGG-Face", enforce_detection=False)
        ...
        if distance < threshold:
            ...
            return jsonify({"status": "failed", "message": "Autentikasi gagal"})
    except Exception as e:
